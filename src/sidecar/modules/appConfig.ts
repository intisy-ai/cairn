...
function realProbe(bundlePath: string): Promise<PluginConfigSchema | null> {
  return new Promise((resolve) => {
    execFile("node", [bundlePath, "config", "schema"], { timeout: 10000 }, (error, stdout) => {
      ...
      try {
        const data = JSON.parse(stdout.trim()) as { name?: unknown; defaults?: unknown; current?: unknown; fields?: unknown; actions?: unknown; sections?: unknown; data?: unknown; screens?: unknown };
        ...
        if (Array.isArray(data.screens)) schema.screens = data.screens as PluginConfigSchema["screens"];
