...
function writeManifest(plugin: string, manifest: unknown, files: Record<string, string> = {}): void {
  const repo = join(home, "repos", plugin);
  mkdirSync(repo, { recursive: true });
  writeFileSync(join(repo, "cairn.json"), JSON.stringify(manifest));
...
describe("readPluginManifest", () => {
  it("resolves the plugin mark and each provider mark to a data URI", () => {
    writeManifest("antigravity-auth", {
      ...
      icon: "icon.svg",
      providers: { antigravity: "icon.svg", "gemini-cli": "icons/gemini-cli.svg" },
  ...
  it("drops a mark whose file is missing rather than emitting a broken source", () => {
    writeManifest("p", { icon: "icon.svg", providers: { one: "gone.svg" } }, { "icon.svg": SVG });
  ...
  it("ignores a providers entry that is not an object of paths", () => {
    writeManifest("p", { providers: ["nope"] });
